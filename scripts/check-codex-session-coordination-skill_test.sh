...
function handleRequest(request) {
  ...
  if (request.method === "thread/read") {
    ...
    respond(request.id, { thread: match });
    return;
  }
...
if (process.env.FAKE_QUEUE_CAPTURE) {
  writeFileSync(process.env.FAKE_QUEUE_CAPTURE, JSON.stringify(process.argv.slice(2)));
}
if (mode === "queue-fail") {
...
jq -e --arg target_id "${target_id}" '.status == "queued" and .targetThreadId == $target_id and .exitCode == 0' \
